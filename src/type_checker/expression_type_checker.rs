...
pub fn check_expr(exp: &Expression, env: &Environment<Type>) -> Result<Type, ErrorMessage> {
    match exp {
        ...
        Expression::FuncCall(func_name, exp_vec) => check_func_call(func_name, exp_vec, env),
...
fn func_to_type(func: &Function) -> Type {
    let arg_types = func
        .params
        .iter()
        .map(|formal_arg| formal_arg.argument_type.clone())
        .collect();
...
fn check_func_call(
    func_name: &Name,
    exp_vector: &[Expression],
    env: &Environment<Type>,
) -> Result<Type, ErrorMessage> {
    let mut actual_arg_types = Vec::new();

    for arg in exp_vector {
        match arg {
